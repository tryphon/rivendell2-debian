...
//
//      $Id: livewire_gpio.cpp,v 1.1.2.6 2013/03/09 00:21:17 cvs Exp $
//
...
LiveWireGpio::LiveWireGpio(RDMatrix *matrix,QObject *parent,const char *name)
  : Switcher(matrix,parent,name)
{
  ...
  // GPIO Socket
  //
  if((livewire_gpio_socket=socket(PF_INET,SOCK_DGRAM,0))<0) {
    syslog(LOG_ERR,"unable to create GPIO socket [%s]",strerror(errno));
    return;
  }
  sockopt=O_NONBLOCK;
  fcntl(livewire_gpio_socket,F_SETFL,sockopt);
  memset(&sa,0,sizeof(sa));
  sa.sin_family=AF_INET;
  sa.sin_port=htons(RD_LIVEWIRE_GPIO_RECV_PORT);
  sa.sin_addr.s_addr=htonl(INADDR_ANY);
  if(bind(livewire_gpio_socket,(struct sockaddr *)&sa,sizeof(sa))<0) {
    syslog(LOG_ERR,"unable to bind GPIO socket [%s]",strerror(errno));
    return;
  }
  livewire_gpio_notify=
    new QSocketNotifier(livewire_gpio_socket,QSocketNotifier::Read,this);
...
LiveWireGpio::~LiveWireGpio()
{
  if(livewire_gpio_notify!=NULL) {
    close(livewire_gpio_socket);
...
RDMatrix::Type LiveWireGpio::type()
{
  return RDMatrix::LiveWireGpio;
}


unsigned LiveWireGpio::gpiQuantity()
{
  return livewire_gpios;
}


unsigned LiveWireGpio::gpoQuantity()
{
  return livewire_gpios;
}


bool LiveWireGpio::primaryTtyActive()
{
  return false;
}


bool LiveWireGpio::secondaryTtyActive()
{
  return false;
}


void LiveWireGpio::processCommand(RDMacro *cmd)
{
  int slot;
  int line;
...
void LiveWireGpio::gpioActivatedData(int sock)
{
  int n;
  char data[1500];
  struct sockaddr_in sa;
  ...
  while((n=recvfrom(livewire_gpio_socket,data,1500,MSG_DONTWAIT,(struct sockaddr *)(&sa),&sa_len))>0) {
...
void LiveWireGpio::gpiTimeoutData(int gpi)
{
  emit gpiChanged(livewire_matrix,gpi,false);
}


void LiveWireGpio::gpoTimeoutData(int gpo)
{
  int slot=gpo/5;
  int line=gpo%5;
...
void LiveWireGpio::ProcessGpi(const QHostAddress &src_addr,int chan,
...
void LiveWireGpio::ProcessGpo(int chan,unsigned line,bool state)
{
  //
  // Destination Address
  //
  ...
  */
  ...
  data[27]=0xFF&state;
  sendto(livewire_gpio_socket,data,28,MSG_DONTWAIT,
...
QString LiveWireGpio::AddressString(uint32_t addr) const
{
...
void LiveWireGpio::subscribe(const QHostAddress &addr) const
{
  subscribe(htonl(addr.toIPv4Address()));
}


void LiveWireGpio::subscribe(const uint32_t addr) const
{
  ...
  mreq.imr_ifindex=0;
  if(setsockopt(livewire_gpio_socket,SOL_IP,IP_ADD_MEMBERSHIP,&mreq,sizeof(mreq))!=0) {
    if(errno!=EADDRINUSE) {
...
void LiveWireGpio::unsubscribe(const QHostAddress &addr) const
{
  unsubscribe(htonl(addr.toIPv4Address()));
}


void LiveWireGpio::unsubscribe(const uint32_t addr) const
{
  struct ip_mreqn mreq;
  ...
  mreq.imr_address.s_addr=htonl(livewire_interface_addr.toIPv4Address());
  mreq.imr_ifindex=0;
  if(setsockopt(livewire_gpio_socket,SOL_IP,IP_DROP_MEMBERSHIP,&mreq,
		sizeof(mreq))!=0) {
    if(errno!=ENODEV) {
